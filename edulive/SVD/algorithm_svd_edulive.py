...
def _run_epoch_baseline_implicit(X, pu, qi, pu_a, qi_a, global_mean_, pen, lr, reg,  lambda1 , lambda2):
    ...
    n_shape = X.shape[0]
    for i in range(n_shape):
        user, item, rating, time_exam, action = int(X[i, 0]), int(X[i, 1]), X[i, 2], X[i,3], X[i,4]
