...
def _tu_and_edulive_time(X,X_test,alpha,beta):
    ...
    X_test = pd.merge(X_test,pd.to_datetime(X.groupby('u_id')["t"].mean()), on= ["u_id"],how="left")
    X_test.columns=["u_id","i_id","rating","t","time_exam", "action_exam","tu"]
    X_test['edulive_time']=alpha*(np.log(1+np.abs((X_test["t"]-X_test["tu"]).astype('timedelta64[D]'))**beta))
