...
val = pd.read_csv(path_file_test, sep=",",
                    names=["u_id", "i_id", "rating"])
...
#################################################################################
# algorithm = koren :  loss, rmse, process_time - koren
# algorithm = koren_average : loss, rmse, process_time - koren_average
# algorithm = koren_bubi_average :  loss, rmse, process_time - koren_bubi_average
...
def avaluate_edulive(file, algorithm='edulive'):
    ...
    if algorithm == 'edulive':
        return algorithm_svd_edulive._compute_val_metrics(user_item_rating_from_val_set,pu, qi, pu_a, qi_a, global_mean,mod="nomod", std_=0)
...
print(avaluate_edulive(file,algorithm='edulive'))